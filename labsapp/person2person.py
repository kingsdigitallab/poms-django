...
# Idea: 
# 
# use JIT to show person to person networks. What they will have in common is some factoid (transactions), what is highlighted is also the role the person plays in a factoid (reified as a node for now)
...
# 2012-08-10: I dropped development on this graph library.. moved on to sankey graph for now
	...
	def build_graph(p1, p2, factoids):
		...
		for p in [p1, p2]:	
			...
			for f in factoids:
				...
				for r in roles:
					r1 = {}
					r1['id'] =	"role_%d%d%d" % (f.id, p.id, r.id) # so to make it unique
					r1['name'] = r.name
					r1['data'] = {	"$color": "green", "$type": "circle", "$dim": 3}
					r1['adjacencies'] = [{"nodeTo": "factoid_%d" % f.id, "nodeFrom": "role_%d%d%d" % (f.id, p.id, r.id), "data": { "$color": "#557EAA" }}]	
					stuff += [r1]
					
					# update the person 2 role link too
					d1['adjacencies'] += [ {"nodeTo": "role_%d%d%d" % (f.id, p.id, r.id), "nodeFrom": "agent_%d" % p.id, "data": { "$color": "#557EAA" }}]	
