...
# Idea: 
# 
# use a Sankey diagram <http://bost.ocks.org/mike/sankey/> to show person to person networks. What they will have in common is some factoid (transactions), what is highlighted is also the role the person plays in a factoid (reified as a node for now)
...
# IMPORTANT: the way the links are structured, must have an implicit 'flow' orientation (= no symmetrical links)
...
def index(request, template = None):
	...
	def build_graph(p1, p2, factoids):
		...
		def indexes_in_list(testlist, item, n = 'None'):
			return [i for i,x in enumerate(testlist) if x == {'name': item, 'id' : n}]
		...
		stuff["nodes"] += [{'name': f.shortdesc, 'id' : factoid_id(f.id)} for f in factoids]
		stuff["nodes"] += [{'name': x, 'id' : role_id(y)} for x, y in p2rols]
		
		for factoid in factoids:
			factoidpos = indexes_in_list(stuff["nodes"], factoid.shortdesc, factoid_id(factoid.id))[0]	

			for x in p1.get_factoidRole(factoid):
				rolpos = indexes_in_list(stuff["nodes"], x.name, role_id(x.id))[0]	# first appearance
				stuff["links"] += [{"source":rolpos,"target":factoidpos,"value":10}]   # value fixed for now

			for x in p2.get_factoidRole(factoid):
				try: 
					rolpos2 = indexes_in_list(stuff["nodes"], x.name, role_id(x.id))[1]	 # get the second appearance, if existing...
				except: 
					rolpos2 = indexes_in_list(stuff["nodes"], x.name, role_id(x.id))[0] 
				
				stuff["links"] += [{"source": factoidpos ,"target": rolpos2,"value":10}]   # value fixed for now			
...
def __get_connections(person_obj):
	...
	for f in person_obj.get_factoids()[:MAX]:
		people += [x.person for x in f.assochelperperson_set.all() if x.person != person_obj]
...
def iteminfo(request):
	...
	"""
	item_id = request.GET.get('item') # item we want info about: Role or Transaction
	if item_id[:2] == "ro":
		# it's a role
		try:
		...
		return_str = """%s <a style="font-size:10px;text-transform: none;" href="/browse/?filter=roles_%s&resulttype=people" title="Search in PoMS" target="_blank">&rarr; Search for all People/Institutions with this role</a></span> """ % (x.name, x.name)
