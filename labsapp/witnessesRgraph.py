...
def index(request, template = None):
	...
	experiment_description = """
				...
				"""	
	obj_id = request.GET.get('id') # getlist gets all parameters with same name!!!!				
	try:
	...
	def build_tree(trans, level=0):
		...
		if level < 1:
			...
			for assoc in trans.assocfactoidwitness_set.all():
				...
				d1['data'] = {}
				d1['children'] = [build_tree(a.factoid.facttransaction, level+1) for a in AssocFactoidWitness.objects.filter(person=witness) if a.factoid.inferred_type == "transaction"]
