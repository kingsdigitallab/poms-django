...
from django.contrib.admin.filterspecs import RelatedFilterSpec, FilterSpec, ChoicesFilterSpec
...
#
#  Mikele: I created this hack for not showing as a filter the non-staff users:
#
#  in reality, what this does is select only the Users explicitly associated to objects, and show them as filters
#  As a side effect, Staff users only are displayed! (cause they're the only ones that created objects)
...
#  and http://my.opera.com/curaloucura/blog/2009/02/17/custom-filter-on-django-admin
#
...
# 		super(StaffUsersSpec, self).__init__(f, request, params, model, model_admin)
...
# 			val = smart_unicode(d[self.field.name])
# 			# print str(val)
# 			#  NOTE:  If the val is not None, get the User name for display purposese
...
#
#		values_list = model.objects.filter(updated_by__is_staff=True).values_list(f.name, flat=True)
