...
class Person(mymodels.PomsModel):
    ...
    def how_many_factoids(self):
        num = 0
        num += len(self.assocfactoidperson_set.all())
        num += len(self.assocfactoidproanima_set.all())
        num += len(self.assocfactoidwitness_set.all())
        return num
    ...
    def how_many_sources(self):
        ...
        l = []
        for x in self.assocfactoidperson_set.all():
            try:
                s = x.factoid.sourcekey
                l.append(s)
            except:
                pass
        for x in self.assocfactoidproanima_set.all():
            try:
                s = x.factoid.sourcekey
                l.append(s)
            except:
                pass
        for x in self.assocfactoidwitness_set.all():
            try:
                s = x.factoid.sourcekey
                l.append(s)
            except:
    ...
    def get_association_factoids(self, whattype="", ordering=None):
        ...
        if not whattype:
            ...
            return list(self.assocfactoidperson_set.all()) + list(self.assocfactoidproanima_set.all()) + list(
                self.assocfactoidwitness_set.all())

        elif whattype in ['possession', 'relationship', 'title/occupation', 'transaction']:
            if not ordering:
                return self.assocfactoidperson_set.filter(factoid__inferred_type=whattype)
            else:
                return self.assocfactoidperson_set.filter(factoid__inferred_type=whattype).order_by(*ordering)

        elif whattype == "proanima":
            if not ordering:
                return self.assocfactoidproanima_set.all()
            else:
                return self.assocfactoidproanima_set.all().order_by(*ordering)

        elif whattype == "witness":
            if not ordering:
                return self.assocfactoidwitness_set.all()
            else:
                return self.assocfactoidwitness_set.all().order_by(*ordering)
        else:
...
class Source(mymodels.PomsModel):
    ...
    def get_factoids(self, whattype="", ordering=None):
        ...
        """
        if not whattype:
            return self.factoid_set.all()
        elif whattype in ['possession', 'relationship', 'title/occupation', 'transaction']:
            if not ordering:
                factoid_set = self.factoid_set.filter(inferred_type=whattype)
            else:
                factoid_set = self.factoid_set.filter(
                    inferred_type=whattype).order_by(*ordering)
            # returns the right factoid type
            return [f.get_right_subclass()[1] for f in factoid_set if f.get_right_subclass]
            ...
            # return ALL  [this supersedes also get_all_associations]
            print
            "Valid factoid types are: 'possession', 'relationship', 'title/occupation', 'transaction'"
            return []
...
class Factoid(mymodels.PomsModel):
    ...
    # 2010-11-17: new helper table to facilitate querying places from factoids
