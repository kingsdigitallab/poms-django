...
class Proanimagenerictypes(mymodels.PomsAuthorityList):
    ...
    newline = models.IntegerField(
        null=True,
        blank=True,
        verbose_name="newline",
    )  # this field should not be needed anymore
...
class Place(mymodels.PomsModel):
    ...
    orderno = models.IntegerField(
        null=True, blank=True, verbose_name="ordering",)
    lft = models.IntegerField(null=True, blank=True, verbose_name="lft?",)
...
class Person(mymodels.PomsModel):
    persondisplayname = models.CharField(
        ...
        null=True,
        blank=True,
        verbose_name="Surface name",
    )
    standardmedievalname = models.CharField(
        max_length=255, null=True, blank=True, verbose_name="Medieval name",)
    ...
    patronym = models.CharField(
        max_length=765,
        verbose_name="Patronym",
        null=True,
        blank=True,
    )
    ofstring = models.CharField(
        max_length=765,
        verbose_name="ofString",
        null=True,
        blank=True,
    )
    placeandinst = models.CharField(
        max_length=765,
        verbose_name="Place/ institutional",
        null=True,
        blank=True,
    )
    ...
    def get_all_associations(self):
        return list(self.assocfactoidperson_set.all(
        )) + list(self.assocfactoidproanima_set.all()) + list(self.assocfactoidwitness_set.all())
...
class Source(mymodels.PomsModel):
    ...
    probabledate = models.CharField(
        ...
        null=True,
        blank=True,
        verbose_name="prob date",
    )
    firmdate = models.CharField(
        max_length=765,
        null=True,
        blank=True,
        verbose_name="firm date",
    )
    ...
    def __unicode__(self):
        ...
        out = "Charter %s/%s/%s %s (%s)" % (self.hammondnumber,
                                            self.hammondnumb2,
                                            self.hammondnumb3,
                                            self.hammondext,
                                            self.source_tradid)
...
class AssocFactoidProanima(mymodels.TimeStampedHiddenModel):
    #factoidpersonkey = models.IntegerField()
    factoidtrans = models.ForeignKey(
        'Factoid', related_name='assocfactoidproanima')
    person = models.ForeignKey('Person')
...
class AssocFactoidPossession(mymodels.TimeStampedHiddenModel):
    ...
    possession_old = models.ForeignKey(
        'PossessionOld', verbose_name="possessions - old classification",)
    possession = models.ForeignKey(
        'Possession',
        null=True,
        blank=True,
        verbose_name="possessions - new temp classification",
    )  # shall we keep this as the default one?
    poss_alms = models.ForeignKey(
        'Poss_Alms',
        null=True,
        blank=True,
        verbose_name="alms",
    ...
    poss_revenues_silver = models.ForeignKey(
        'Poss_Revenues_silver',
        null=True,
        blank=True,
        verbose_name="revenues in silver",
    )
    poss_revenues_kind = models.ForeignKey(
        'Poss_Revenues_kind',
        null=True,
        blank=True,
        verbose_name="revenues in kind",
    )
    poss_general = models.ForeignKey(
        'Poss_General',
        null=True,
        blank=True,
        verbose_name="possessions in general",
    )
    poss_office = models.ForeignKey(
        'Poss_Office',
        null=True,
        blank=True,
        verbose_name="office",
    )
    poss_objects = models.ForeignKey(
        'Poss_Objects',
        null=True,
        blank=True,
        verbose_name="objects",
    )
    poss_lands = models.ForeignKey(
        'Poss_Lands',
        null=True,
        blank=True,
        verbose_name="lands",
...
class AssocFactoidPoss_alms(mymodels.TimeStampedHiddenModel):
    ...
    factoidpos = models.ForeignKey('Factoid')
    poss_alms = models.ForeignKey(
        'Poss_Alms', blank=True, verbose_name="alms",)
...
class Factoid(mymodels.PomsModel):
    ...
    # factoidtypekey = models.IntegerField()   --> not needed cause I have
    # subclasses now
    ...
    # alkey = models.IntegerField() --> this is the 'magic' field we're going
    # to explode into subclasses info
    @models.permalink
    def get_absolute_url(self):
...
class FactTransaction(Factoid):
    ...
    tenendas = models.ManyToManyField(
        ...
        verbose_name="tenendas",
        blank=True)  # through='Ass_TransTenendas',
    exemptions = models.ManyToManyField(
        Exemptiontype,
        verbose_name="exemptions",
        blank=True)  # through='Ass_TransExemption',
    ...
    renderdates = models.ManyToManyField(
        Renderdate,
        verbose_name="render dates",
        blank=True)  # through='Ass_TransRenderDate',
    ...
    legalpertinents = models.ManyToManyField(
        LegalPertinents,
        verbose_name="additional legal pertinents",
        blank=True)
