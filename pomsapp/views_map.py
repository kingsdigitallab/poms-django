...
def map_image(request):
    ...
    # return HttpResponseRedirect('pomsapp/map/map_image.html',context,context_instance=RequestContext(request))


def map_search(request):
    geomPoly = GEOSGeometry('{ "type": "Polygon", "coordinates":' + request.GET.get('selected_geom') + '}}')
    context = {}
    context['places'] = Place.objects.filter(geom__intersects=geomPoly).exclude(parent__isnull=True)
...
def hierarchy(request):
    ...
    places = Place.objects.exclude(parent__isnull=False).filter(name__contains='(')  # .order_by('name')
    places = places | Place.objects.exclude(parent__isnull=False).filter(name__contains='/')
