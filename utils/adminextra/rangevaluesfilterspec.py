...
#
# Adds filtering by ranges of values in the admin
...
class RangeValuesFilterSpec(FilterSpec):
    ...
    def __init__(self, f, request, params, model, model_admin):
        ...
        self.parsed_params = dict(
            [(k, v) for k, v in params.items() if k.startswith(self.field_generic)])  # noqa
        ...
        for max_value in sorted(f.list_filter_range):
            ...
            if last_value is None:
                ...
                range = {
                    '%s__gte' %
                    self.field.name: last_value,
                    '%s__lt' %
                    f.name: max_value}
            self.links.append((_(mark_safe(label)), range))
            last_value = max_value
        self.links.append((_(mark_safe('&ge; ' + max_value)),
                           {'%s__gte' % f.name: max_value}))
