...
# 2. the ordering of resulting people must be controlled  (this could be
# fixed easily though)
...
def index(request):
    ...
    """
    obj_id_list = request.GET.getlist(
    ...
    # The size of the links depends on the number of factoids two agents have
    # in common.
...
def getCommonConnections(request):
    ...
    return HttpResponse("<br />".join(["%s: <a href='%s' target='_blank' title='Open in POMS'>%s</a>" % (
        x.inferred_type.upper(), x.get_absolute_url(), x.shortdesc) for x in common_factoids]))
...
def autocomplete_people(request):
    ...
    if request.method == "GET":
        if u'term' in request.GET:
            ...
            model_results = Person.objects.filter(
                persondisplayname__icontains=value)[:60]
            results = [{'label': x.persondisplayname, 'value': x.id}
