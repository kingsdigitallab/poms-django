...
def scatter_peopleage(request):
    ...
    females = Person.objects.filter(
        genderkey__id=4).exclude(
        floruitstartyr__isnull=True,
        floruitendyr__isnull=True)
    males = Person.objects.filter(
        genderkey__id=3).exclude(
        floruitstartyr__isnull=True,
        floruitendyr__isnull=True)
    unknown = Person.objects.filter(
        genderkey__id=6).exclude(
        floruitstartyr__isnull=True,
        floruitendyr__isnull=True)
    institutions = Person.objects.filter(
        genderkey__id__in=[
            2,
            5]).exclude(
        floruitstartyr__isnull=True,
...
def scatter(request):
    ...
    data3 = [[174.0, 65.6], [175.3, 71.8], [193.5, 80.7], [186.5, 72.6], [187.2, 78.8],
             ...
             [180.0, 76.6], [177.8, 83.6], [192.0, 90.0], [
...
def iteminfo(request):
    ...
    """
    agenttype = request.GET.get(
        'agenttype')  # Females, Males, Unknown or Institutions
    ...
    if agenttype == 'Unknown':
        ...
        pset = Person.objects.filter(genderkey__id__in=[2, 5]).exclude(
            floruitstartyr__isnull=True, floruitendyr__isnull=True)
    ...
    return HttpResponse("<br />".join(["<a target='_blank' name='/labs/popularitycloud/ajax/personinfo?item=%d' href='%s'>%s</a>" % (
        p.id, p.get_absolute_url(), p.persondisplayname) for p in exit]))
