...
def cacheAllFacets(fm, result_types, SLICING=None):
    for facet in fm.get_all_facets():
        for r in result_types:
            v = fm._cacheOneFacetValue(
                facet,
                resulttype=r,
                ENFORCE=True,
                SLICING=SLICING)  # ENFORCE = True
            if v:
                print("\n\nzzzzzzzzzzzzzzzzzz\
                    sleeping 30 seconds zzzzzzzzzzzzzzzzzzzzzzzz\n\n")
                time.sleep(30)
...
def cacheOneFacetAllResults(facet_name, fm, ENFORCE=False, SLICING=None):
    ...
    for r in result_types:
        v = fm._cacheOneFacetValue(
            ...
            resulttype=r,
            ENFORCE=ENFORCE,
            SLICING=SLICING)  # ENFORCE = True, DELETE =True
        if v:
            print("\n\nzzzzzzzzzzzzzzzzzz sleeping\
                30 seconds zzzzzzzzzzzzzzzzzzzzzzzz\n\n")
            time.sleep(30)


def cacheOneFacetOneResult(facet_name, result_name,
                           fm, ENFORCE=False, SLICING=None):
    facet = fm.get_facet_from_name(facet_name)
    ...
    v = fm._cacheOneFacetValue(
        ...
        resulttype=r1,
        ENFORCE=ENFORCE,
        SLICING=SLICING)  # Queryargs, ENFORCE = True, DELETE =True
    if v:
        print("\n\nzzzzzzzzzzzzzzzzzz\
            DONE zzzzzzzzzzzzzzzzzzzzzzzz\n\n")


# technique for caching facets-values with lots of values (that generate a
# mem-error!)
def cache_longestFacets(fm):
    for f in ['surname', 'forename']:
        for t in [(0, 2000), (2000, 4000), (4000, 6000)]:
            try:
                cacheOneFacetOneResult(
                    f, 'factoid', fm, ENFORCE=True, SLICING=t)
                cacheOneFacetOneResult(
                    f, 'source', fm, ENFORCE=True, SLICING=t)
                cacheOneFacetOneResult(
                    f, 'people', fm, ENFORCE=True, SLICING=t)
            except BaseException:
...
def deleteCachedFacet(fm, facet_name, result_name, queryargs=None):
    fm._emptyCacheForFacetQuery(
        fm.get_facet_from_name(facet_name),
        fm.get_resulttype_from_name(result_name),
        queryargs)
...
def deleteAllCachedFacets(fm, queryargs=None):
    ...
    # >>> c.id
    # 37L
    # >>> CachedFacetQuery.objects.get(facet='forename',
    # resulttype='factoid', queryargs=c)
    # <CachedFacetQuery: CachedFacetQuery object>
    # >>> x = CachedFacetQuery.objects.get(facet='forename',
    # resulttype='factoid', queryargs=c)
    # >>> x.delete()
    # >>> x = CachedFacetQuery.objects.get(facet='surname',
    # resulttype='factoid', queryargs=c)
    # >>> x.delete()
...
# finds all cached values that still have more than X
# results; uses them to compose a query, and re-calculates all
# values-count in this new context...
def cache_secondLevel_objects(n, fmanager, test=False):
    cached = CachedFacetValue.objects.filter(count__gt=n).order_by('-count')
    totcount = len(l)
    count = 0
    if test:
        cached = cached[:1]   # TESTING WITH ONE VALUE
    for cachedValue in l:
        ...
        print("\n$$$$$$$$$$$$\nFIND_VALUES_WITH_COUNT_MORETHAN:\n\
            [facetvalue= *%s*] [facet = *%s*][restype = *%s*] with\
            count *%d* ..... caching...\nvalue %d of\
            %d\n$$$$$$$$$$$$\n" % (facetvalue_name,
                                   facet_name, resulttype_name,
                                   cachedValue.count, count, totcount))
        ...
        active_facetvalue = active_facet.get_facetvalue_from_name(
            facetvalue_name)
        active_facetsGroup = None  # facetGroup is not used...

        # QueryArgs: list of [active_facetsGroup, active_facet, facetvalue]
        fmanager.add_queryarg(
            active_facetsGroup,
            active_facet,
            active_facetvalue)

        # we changed the state of fmanager, so the query will cache other
        # stuff!

        # we cache the count for all facets, using that specific resultType
        # where the count is very high.
        for fac in fmanager.get_all_facets():
            print("\n[%s]\n" % strftime("%Y-%m-%d %H:%M:%S"))
            if fac.uniquename == 'surname' or\
                    fac.uniquename == 'forename' or fac.uniquename == 'titles':
                for t in [(0, 1000), (1000, 2000), (2000, 3000),
                          (3000, 4000), (4000, 5000)]:
                    try:
                        cacheOneFacetOneResult(
                            fac.uniquename, resulttype_name,
                            fmanager, SLICING=t)  # ENFORCE=True,
                    except BaseException:
                        break  # in case the slicing fails...
            else:
                cacheOneFacetOneResult(
            ...
            print("\n[%s]\nzzzzzzzzzzzzzzzzzz waiting\
                  1 second zzzzzzzzzzzzzzzzzzzzzzzz\n" %
                  strftime("%Y-%m-%d %H:%M:%S"))
            time.sleep(1)

            # v = fmanager._cacheOneFacetValue(fac,
            # active_resulttype)  #ENFORCE=True
            # if v:
